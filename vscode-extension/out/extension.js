...
function activate(context) {
    const disposable = vscode.commands.registerCommand('nestify.openChat', () => {
        const panel = vscode.window.createWebviewPanel('nestifyChat', 'Nestify Chat', vscode.ViewColumn.Active, { enableScripts: true });
        panel.webview.html = getHtml();
        panel.webview.onDidReceiveMessage(async (msg) => {
            if (msg.type === 'send') {
                const pythonPath = vscode.workspace.getConfiguration().get('nestify.pythonPath', 'python');
                const proc = (0, child_process_1.spawn)(pythonPath, ['-m', 'nestify_bridge']);
